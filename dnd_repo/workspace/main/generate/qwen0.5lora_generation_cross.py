...
def main(eval_dataset: str):
    # Model
    print("==> Building model..")
    diction = torch.load(f"/root/autodl-tmp/{model_type}__{eval_dataset}.pth", weights_only=True, map_location="cpu")
    ...
    test_set = Dataset(
        ...
        texts=[json.load(open(f"{COND_ROOT}/science-dataset.json", "r", encoding="utf-8"))],
    ...
    print(f"\n\nGenerating parameters for science-dataset")
    generate(model, test_loader, test_set, eval_dataset, "science-dataset")
    del model, test_loader
    torch.cuda.empty_cache()
    gc.collect()

    process_adapter_path(f"{SAVE_ROOT}/{eval_dataset}T_on_science-datasetV")
    print("==> Start testing..")
    os.chdir("./prepare")
    ...
    for i in range(config["real_length"]):
        ...
        args = [
            ...
            "--save_name",
            f"{RES_ROOT}/science-dataset/{eval_dataset}T_on_science-datasetV_{i}.jsonl",
            "--dataset",
            f"science-dataset",
            "--adapter_name_or_path",
            f"{TEST_ROOT}/{eval_dataset}T_on_science-datasetV_{i}",
        ]
        subprocess.run(["python", "scripts/vllm_infer.py"] + args)
        subprocess.run(
            ["python", "scripts/calculate_acc.py"]
            + ["--file", f"{RES_ROOT}/science-dataset/{eval_dataset}T_on_science-datasetV_{i}.jsonl"]
        )
